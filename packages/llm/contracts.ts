...
export function buildPlanningPrompt(
  ...
  return [
    ...
    'If ExtraContext contains observedPaths or pathObservations, treat them as the only confirmed discovery results for this task and prefer them over guessed file or directory names.',
    `ExecutionProfile: ${executionProfile}`,
    '',
    `Goal: ${goal}`,
    ...
    '- if a discovery search returns zero matches, do not continue from that empty result; choose a more concrete identifier from the goal or switch to another discovery tool',
    '- after repo.search_text, repo.search_files, repo.symbol_search, or fs.list_dir, reuse confirmed candidate paths from observedPaths/pathObservations instead of inventing sibling directories or file names',
